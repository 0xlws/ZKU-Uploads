...
// implementation of mempool transaction queue
// this implementation is used by state.rs where the transaction is 
// is going trough various checks e.g. if account linked to
// transaction is valid. it then gets split into 
// priority operations (operations initiated from the L1)
// or zkSync signed txs (transactions with the corresponding Ethereum signature and the message)
impl MempoolTransactionsQueue {
    ...
    // these functions removes the first element from the queue and returns it
    ...
    // push l2 transaction containing tx details into queue 
    ...
    // prepare l2 transactions and if valid removes from pending to ready queue
