...
contract LoanDapp is IAZTEC {
  ...
  // return the currency used for the loan only if defined
  ...
  // function to generate accessId for when requested via other internal function
  ...
  // initiate new loan (Loan.sol), called by createLoan function
  // updates contract state and mints a note after validating proofdata
  // returns the address of the new loan created
  ...
  // owner of the contract can add valid ERC20 token as settlement currency
  ...
  // funciton being called to create a new loan 
  // gives the caller viewing access to the note
  function createLoan(
    bytes32 _notional,
  ...
  // approve/sign the notional of the loan
  ...
  // only the borrower can approve the request of the loan to be viewed
  ...
  // if correct viewingKey and sharedSecret provided allows access to view the note 
